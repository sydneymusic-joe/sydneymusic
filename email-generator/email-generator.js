...
async function main () {
    ...
    for (const month of gigs) {
        ...
        for (const day of month.items) {
            template += `
            <tr class="day">
                <td valign="top" align="center" width="80" class="daylabel"><strong style="text-transform : uppercase;" class="ruby">${day.label.split(':')[1]}</strong><br /><font style="font-size : 36px; font-weight : bold">${day.label.split(':')[0]}</font></td>
                <td>
            ...
            for (const gig of day.items) {
                ...
                if (gig.performersList != null) {
                    template += 
                    `<strong>w/ ${gig.performersList.join(", ")}</strong><br />`;
                }
                        template += `${gig.time} &nbsp;| &nbsp;<a href="${gig.venue.url}">${gig.venue.venueName}</a>`;
                        if (gig.furtherInfo) {
                            template += `<div class="blurb">${gig.furtherInfo} &mdash; ${gig.furtherInfoContributorInitials ? gig.furtherInfoContributorInitials : "SMn"}</div>`
                        }
                        template += `</td>
                </tr>`;
            }
